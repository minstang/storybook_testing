...
function methodHandler(documentation, path) {
    if (bt.isObjectExpression(path.node)) {
        var methodsPath = path
            .get('properties')
            .filter(function (p) { return bt.isObjectProperty(p.node) && p.node.key.name === 'methods'; });
        // if no method return
...
function setMethodDescriptor(methodDescriptor, method, jsDocTags) {
    // params
    describeParams(method, methodDescriptor, jsDocTags.filter(function (tag) { return tag.title === 'param'; }));
    // returns
...
function describeParams(methodPath, methodDescriptor, jsDocParamTags) {
    ...
    fExp.params.forEach(function (par, i) {
        var param = { name: par.name };
        var jsDocTags = jsDocParamTags.filter(function (tag) { return tag.name === param.name; });
