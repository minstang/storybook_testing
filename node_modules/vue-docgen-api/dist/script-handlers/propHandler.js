...
function propHandler(documentation, path) {
    if (bt.isObjectExpression(path.node)) {
        var propsPath = path
            .get('properties')
            .filter(function (p) { return bt.isObjectProperty(p.node) && p.node.key.name === 'props'; });
        // if no prop return
        if (!propsPath.length) {
            return;
        }
        ...
        if (bt.isObjectExpression(propsValuePath.node)) {
            ...
            objPropFiltered.forEach(function (prop) {
                ...
                var propName = jsDocTags.some(function (t) { return t.title === 'model'; }) ? 'v-model' : propNode.key.name;
...
function describeType(propPropertiesPath, propDescriptor) {
    var typeArray = propPropertiesPath.filter(function (p) { return p.node.key.name === 'type'; });
    if (typeArray.length) {
        propDescriptor.type = getTypeFromTypePath(typeArray[0].get('value'));
    }
    else {
        // deduce the type from default expression
        var defaultArray = propPropertiesPath.filter(function (p) { return p.node.key.name === 'default'; });
...
function getTypeFromTypePath(typePath) {
    ...
    var typeName = bt.isArrayExpression(typeNode)
        ? typePath
            .get('elements')
...
function describeRequired(propPropertiesPath, propDescriptor) {
    var requiredArray = propPropertiesPath.filter(function (p) { return p.node.key.name === 'required'; });
