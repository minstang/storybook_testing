...
function isMethod(path) {
  const isProbablyMethod = t.MethodDefinition.check(path.node) && path.node.kind !== 'constructor' || (t.ClassProperty.check(path.node) || t.Property.check(path.node)) && t.Function.check(path.get('value').node);
